...
- **UTXO Multi-Hop Flow Tracing**:
  ...
  - Aggregates multi-transfer volume between addresses to eliminate graph clutter.
...
- **Interactive Investigation Tools**:
  ...
  - **Known Entity Tagging**: Pre-indexed database for major exchanges (MEXC, KuCoin, Gate.io), mining pools (ViaBTC, HumPool), and Kaspa Dev Fund.
...
```bash
# Clone repository
git clone <your-repo-url> dag-recon
cd dag-recon
...
│   ├── engine/
│   │   ├── canvasRenderer.ts     # 2D Canvas renderer, particles & minimap
...
│   ├── components/
│   │   ├── TopNav.tsx            # Search bar, presets & scan triggers
│   │   ├── CanvasGraph.tsx       # Canvas container with pan/zoom/context menu
│   │   ├── Inspector.tsx         # Node/edge forensic inspection sidebar
│   │   ├── TerminalLog.tsx       # Collapsible cyberpunk telemetry event stream
│   │   ├── FilterBar.tsx         # Attack filters & collapsible color legend
...
│   │   └── InfoModals.tsx        # Guide & Kaspa donation modals
